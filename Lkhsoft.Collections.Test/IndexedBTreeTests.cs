...
[TestFixture]
public class IndexedBTreeTests
{
    [Test]
    public void TestSearchPerformance_ShouldWork()
    {
        ...
        var i = 1;
        GC.Collect();
        var btree = new IndexedBTree<int, string>(128);
        ...
        while (i <= numberOfKeys)
        {
            ...
            while (j <= valuesPerKey)
            {
                ...
                TestContext.Out.WriteLine(
                    $"Temps d'exécution de l'ajout de la valeur Value-{i}-{j} : {valueAddStopwatch.ElapsedMilliseconds} ms");
                j++;
            }

            addValuesStopwatch.Stop();
            TestContext.Out.WriteLine(
                $"Temps d'exécution de l'ajout des valeurs pour la clé {i}: {addValuesStopwatch.ElapsedMilliseconds} ms");
            i++;
        }
        ...
        for (var j = 1; j <= valuesPerKey; j++) valuesToSearch.Add($"Value-{keyToSearch}-{j}");
        ...
        Assert.That(foundValues, Is.EquivalentTo(valuesToSearch),
            "Les valeurs trouvées doivent correspondre exactement aux valeurs insérées.");
    ...
    {
        var bTree = new IndexedBTree<int, string>
        {
            {1, "value1"},
            {2, "value2"}
        };
    ...
    {
        var bTree = new IndexedBTree<int, string> {{1, "value1"}};
